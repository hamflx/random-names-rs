...
impl RandomName {
    ...
    pub fn new() -> RandomName {
        RandomName {
            name: create_name(&mut OsRng::default()),
        }
    }
}
...
pub fn create_name<T: Rng>(rng: &mut T) -> String {
    let name: &str = NAMES.choose(rng).map(|x| *x).unwrap_or(NO_NAMES_NAME);
    let surname: &str = SURNAMES.choose(rng).map(|x| *x).unwrap_or(NO_NAMES_SURNAME);
    format!("{} {}", name, surname)
}
